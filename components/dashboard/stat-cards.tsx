'use client'

import { Activity, Cpu, ShieldCheck, TrendingDown, TrendingUp, Users, type LucideIcon } from 'lucide-react'

import { CountUp, StaggerGroup, StaggerItem, hoverCardClass } from '@/components/motion/primitives'
import { Skeleton } from '@/components/ui/skeleton'
import { cn } from '@/lib/utils'
import type { DashboardStats, Trend } from '@/lib/dashboard/types'

type Tile = {
  id: string
  label: string
  icon: LucideIcon
  iconClass: string
  value: string
  trend: Trend
  note: string
  trendNote: string
  operational?: boolean
}

function buildTiles(stats: DashboardStats): Tile[] {
  return [
    {
      id: 'active-users',
      label: 'Active Users',
      icon: Users,
      iconClass: 'bg-[#FFFAEC] text-[#D4A017] border-[#F5CA50]/30',
      value: stats.activeUsers.value.toLocaleString('en-US'),
      trend: stats.activeUsers.trend,
      note: `of ${stats.activeUsers.totalUsers.toLocaleString('en-US')} total`,
      trendNote: 'vs previous period',
    },
    {
      id: 'pending-workflows',
      label: 'Pending Workflows',
      icon: Activity,
      iconClass: 'bg-indigo-50 text-indigo-600 border-indigo-200/60',
      value: stats.pendingWorkflows.value.toLocaleString('en-US'),
      trend: stats.pendingWorkflows.trend,
      note: `${stats.pendingWorkflows.scheduled} scheduled`,
      trendNote: 'vs previous period',
    },
    {
      id: 'ai-requests',
      label: 'AI Requests Today',
      icon: Cpu,
      iconClass: 'bg-orange-50 text-orange-600 border-orange-200/60',
      value: stats.aiRequestsToday.value.toLocaleString('en-US'),
      trend: stats.aiRequestsToday.trend,
      note: 'vs yesterday',
      trendNote: 'vs yesterday',
    },
    {
      id: 'uptime',
      label: 'Success Rate (30d)',
      icon: ShieldCheck,
      iconClass: 'bg-green-50 text-green-600 border-green-200/60',
      value: `${stats.uptime.pct}%`,
      trend: null,
      note: stats.uptime.operational ? 'Operational' : 'Degraded',
      trendNote: '',
      operational: stats.uptime.operational,
    },
  ]
}

const pillClass = 'inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs font-medium'
const positivePill = 'border-emerald-200/60 bg-emerald-50 text-emerald-700'
const negativePill = 'border-rose-200/60 bg-rose-50 text-rose-700'
const neutralPill = 'border-neutral-200/60 bg-neutral-50 text-neutral-600'

function TrendPill({ tile }: { tile: Tile }) {
  if (tile.operational !== undefined) {
    return (
      <span className={cn(pillClass, tile.operational ? positivePill : negativePill)}>
        <span className={cn('size-1.5 rounded-full', tile.operational ? 'bg-emerald-500' : 'bg-rose-500')} />
        {tile.note}
      </span>
    )
  }

  if (!tile.trend) {
    return <span className={cn(pillClass, neutralPill)}>{tile.note}</span>
  }

  const up = tile.trend.direction === 'up'
  const TrendIcon = up ? TrendingUp : TrendingDown
  return (
    <span className={cn(pillClass, up ? positivePill : negativePill)}>
      <TrendIcon className="size-3 shrink-0" />
      {up ? '+' : '-'}{tile.trend.pct}% {tile.trendNote}
    </span>
  )
}

export function StatCards({ stats }: { stats: DashboardStats }) {
  return (
    <StaggerGroup className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4" stagger={0.07}>
      {buildTiles(stats).map((tile) => {
        const Icon = tile.icon
        return (
          <StaggerItem key={tile.id} className="h-full">
            <div
              className={cn(
                'flex h-full flex-col justify-between rounded-xl border border-neutral-200/80 bg-white p-4 shadow-xs',
                hoverCardClass,
              )}
            >
              <div className="flex items-center justify-between gap-2">
                <span className="text-[11px] font-bold tracking-wider text-neutral-500 uppercase">
                  {tile.label}
                </span>
                <div className={cn('shrink-0 rounded-lg border p-1.5', tile.iconClass)}>
                  <Icon className="size-4" />
                </div>
              </div>

              <div className="my-2">
                <span className="text-2xl font-bold tracking-tight text-neutral-900">
                  <CountUp value={tile.value} />
                </span>
              </div>

              <div>
                <TrendPill tile={tile} />
              </div>
            </div>
          </StaggerItem>
        )
      })}
    </StaggerGroup>
  )
}

export function StatCardsSkeleton() {
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="rounded-xl border border-neutral-200/80 bg-white p-4 shadow-xs">
          <div className="flex items-center justify-between">
            <Skeleton className="h-3 w-24" />
            <Skeleton className="size-7 rounded-lg" />
          </div>
          <Skeleton className="my-2 h-8 w-24" />
          <Skeleton className="h-5 w-32 rounded-full" />
        </div>
      ))}
    </div>
  )
}
